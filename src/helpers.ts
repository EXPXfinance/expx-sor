...
export function getLimitAmountSwap(
    ...
    if (swapType === 'swapExactIn') {
        return eMul(poolPairData.balanceIn, MAX_IN_RATIO);
    } else {
        return eMul(poolPairData.balanceOut, MAX_OUT_RATIO);
    }
}
...
export function getSpotPricePath(
    ...
    if (swaps.length == 1) {
        ...
        return eMul(poolPairDataSwap1.sp, poolPairDataSwap2.sp);
    } else {
        throw new Error('Path with more than 2 swaps not supported');
    }
}
...
export function getSpotPrice(poolPairData: PoolPairData): BigNumber {
    let inRatio = eDiv(poolPairData.balanceIn, poolPairData.weightIn);
    let outRatio = eDiv(poolPairData.balanceOut, poolPairData.weightOut);
    if (outRatio.isEqualTo(bnum(0))) {
        return bnum(0);
    } else {
        return eDiv(eDiv(inRatio, outRatio), EONE.minus(poolPairData.swapFee));
    }
}
...
export function getSlippageLinearizedSpotPriceAfterSwapPath(
    ...
    if (swaps.length == 1) {
        ...
        if (
            ...
            let numerator1 = eMul(
                eMul(
                    eMul(EONE.minus(p1.swapFee), EONE.minus(p2.swapFee)), // In mathematica both terms are the negative (which compensates)
                    p1.balanceOut
                ),
                eMul(p1.weightIn, p2.weightIn)
            );

            let numerator2 = eMul(
                eMul(
                    p1.balanceOut.plus(p2.balanceIn),
                    EONE.minus(p1.swapFee) // In mathematica this is the negative but we add (instead of subtracting) numerator2 to compensate
                ),
                eMul(p1.weightIn, p2.weightOut)
            );

            let numerator3 = eMul(
                p2.balanceIn,
                eMul(p1.weightOut, p2.weightOut)
            );
            ...
            if (swapType === 'swapExactIn') {
                let denominator1 = eMul(p1.balanceIn, p1.weightOut);
                let denominator2 = eMul(p2.balanceIn, p2.weightOut);

                return eDiv(eDiv(numerator, denominator1), denominator2);
            } else {
                let denominator1 = eMul(
                    EONE.minus(p1.swapFee),
                    eMul(p1.balanceOut, p1.weightIn)
                );
                let denominator2 = eMul(
                    EONE.minus(p2.swapFee),
                    eMul(p2.balanceOut, p2.weightIn)
                );
...
export function getSlippageLinearizedSpotPriceAfterSwap(
    ...
    if (swapType === 'swapExactIn') {
        if (balanceIn.isEqualTo(bnum(0))) {
            ...
            return eDiv(
                eMul(EONE.minus(swapFee), eDiv(weightIn, weightOut)).plus(EONE),
                balanceIn
            );
        }
    } else {
        if (balanceOut.isEqualTo(bnum(0))) {
            ...
            return eDiv(
                eDiv(weightOut, eMul(EONE.minus(swapFee), weightIn)).plus(EONE),
                balanceOut
            );
        }
    }
...
export function getNormalizedLiquidity(poolPairData: PoolPairData): BigNumber {
    let { weightIn, weightOut, balanceIn, balanceOut, swapFee } = poolPairData;
    return eDiv(eMul(balanceOut, weightIn), weightIn.plus(weightOut));
}
